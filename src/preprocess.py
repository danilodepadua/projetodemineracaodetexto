...
def _write_representation(run_dir: Path, name: str, datasets: dict[str, pd.DataFrame]) -> dict[str, Any]:
    ...
    built = build_representation(name, _texts(datasets["train"]), _texts(
        datasets["valid"]), _texts(datasets["test"]))
    ...
    return {
        ...
        "shapes": {
            split: list(_matrix_shape(matrix))
            for split, matrix in matrices.items()
        },
    }
...
def run(data_dir: Path, output_dir: Path, representation: str = "all") -> Path:
    ...
    cleaned_datasets = {name: clean_dataset(
        dataset) for name, dataset in datasets.items()}
    run_dir = output_dir / \
    ...
    representations = {name: _write_representation(
        run_dir, name, cleaned_datasets) for name in representation_names}
    manifest = {"input_hashes": hashes, "row_counts": {name: len(dataset) for name, dataset in cleaned_datasets.items()}, "documented_markers": MARKERS, "clean_text_column": "essay_clean",
                "representations": representations, "python": platform.python_version(), "pandas": pd.__version__, "scipy": scipy.__version__, "scikit_learn": sklearn.__version__}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/preprocessing"),
                        help="Directory where preprocessing runs are written.")
    parser.add_argument("--representation", choices=[
                        "tf", "tfidf", "all"], default="all", help="Representation to build (default: all).")

...
def test_train_only_fit_and_reload(tmp_path):
    result = build_representation("tfidf", ["Alpha alpha", "Alpha beta"], ["validationonly"], ["testonly"])
    assert "validationonly" not in result.vectorizer.vocabulary_
    path = tmp_path / "vectorizer.joblib"; joblib.dump(result.vectorizer, path)
    assert joblib.load(path).transform(["Alpha"]).shape[1] == result.train.shape[1]
